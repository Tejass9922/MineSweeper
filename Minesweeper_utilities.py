...
def advanced_solver(KB,minMap,d,n,visited,inferenced_cells,clue_counter,i):
    identified_mines = []
    tripped_mines = []
    #visited = set()
    ...
    if i >= n:
          return (minMap,KB,identified_mines,tripped_mines,i,visited,inferenced_cells,clue_counter)
...
def basic_solver_play_by_play(minMap, d, n):
    ...
    while (i < n):
        ...
        input("HIT ENTER FOR NEXT ITERATION ---------------------------------------------------------------------------->")
        printTimeSteps(minMap,d)
            ...
            if cell.visited == 1 and not cell.location in inferenced_cells:
                ...
                if (cell.clue - cell.revealedMines == cell.numHiddenSquares):
                    ...
                    for c in neighbors:
                        ...
                        if c.visited == 0 :
                            c.visited = 1
                            c.clue = getClue(minMap,c)
                            c.revealedMines =  revealedMines(minMap, c)
                            c.numHiddenSquares =  hiddenCells(minMap, c)
                            neighbors = getNeighbors(minMap,c)
                            c.numSafeNeighbors = revealedSafeNeighbors(minMap, c)
                            #inferenced_cells.add(c.location)
                    inferenced_cells.add(cell.location)
                    revealed = True
